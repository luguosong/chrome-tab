...
> **注记(2026-08-26)**:「iconScale 是图标大小的唯一调节」撤除用户调节入口——调节引发图标展示不稳定,系数钉死为代码常量 `ICON_SCALE = 1`;字段、后端校验与 DB 列彻底删除(线上存量值恰为 1,无视觉跳变),几何推导仍真实生效。见 ADR-0033。

> **注记(2026-08-23g)**:「一律占 1 格」修订——类型可在注册表声明 `size` 跨格(AIHOT 3×2 首例),网格 8×8 扩容 9×9。见 ADR-0021;「无用户可调尺寸档位」的精神不变(跨格是类型固有形态,非用户设置)。
