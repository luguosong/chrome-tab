...
1. **原语**:`get(key)`(TTL 命中回缓存不发上游;失败 console.warn + 回落 lastGood——键级、永不过期、含过期缓存;从未成功回 null)/ `invalidate`(写后失效,只清 TTL 缓存——lastGood 是底不清)/ `lastError`(最近失败原始因,成功即清——域选上抛时透传)/ `onSuccess` 域钩子(趋势榜补译 fire-and-forget,异常自吞不牵连取数)。失败不续 TTL:下次调用即重试。键粒度由域选(单值源常量键、组合源序列化键)。
2. **迁移三域**:aihot(createCachedSource 外壳只留匿名 UA 外呼与响应裁剪)、dida(单键,速记/点掉后 invalidate 强制下读重拉)、trending(键 `since|lang|spoken` 序列化 + fetch 回调反解 + onSuccess 补译 + 从未成功域选上抛,lastError 透原始「风控」因)。
