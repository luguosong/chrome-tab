...
- [x] 打开/关闭(点外部)、点子图标默认行为后关闭
- [x] 点名称改名(任意模式;清空回落默认)
- [x] 9/页流式分页 + 滚轮翻页(≤9 吃掉滚轮,不透传走马灯)+ 页点指示器
- [x] portal 在根 DndContext 子树内;backdrop 常态 `pointer-events:none`、内容区恢复交互
- [x] 编辑模式:组内排序(SortableContext)+ 拖出到页面网格(move parentId=null,按保留 size 落位计容量)+ 子图标 ×
- [x] onDragOver 判空/同容器守卫;onDragEnd 关弹层;onDragCancel 回滚且弹层保持开(ESC 拖拽中经合成 pointercancel 走 onDragCancel——dnd-kit 6.x 指针传感器不监听 ESC,见 GroupOverlay 注释)
- [x] Vitest:组内分页切片纯函数(落 `groupReducer.ts`,07 已建;`[9k, 9k+9)` 边界:恰好 9 / 10 / 18 / 19 个)
