...
import { extractContent, isCandidateExhausted, LLM_BASE_URL, modelCandidates } from './changelog'

/**
 * 跨域批量 LLM 译制机制(CONTEXT.md「译文表」;ADR-0029 首建于新闻标题、ADR-0030
 * 复用于趋势描述):候选模型链 + 宁原文勿空;批量编号列表 ≤20 条/请求、批间串行
 * (free 渠道限流敏感,不并发,changelog 同纪律);漏行/畸行返回 null 由调用方依
